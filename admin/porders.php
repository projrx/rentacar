...
<head>
  ...
  if(isset($_POST['oid'])){
    ...
    $paid=$_POST['payment'];
        ...
        $rows =mysqli_query($con,"SELECT carid,days FROM orders where id=$id" ) or die(mysqli_error($con));
        while($row=mysqli_fetch_array($rows)){$carid = $row['carid'];$days = $row['days'];}


            $date= date("Y-m-d");
            $dates =date('Y-m-d', strtotime($date. ' + '.$days.' days'));

            $enddate =date('Y-m-d', strtotime($dates. ' - 1 days')); 
 
            $status = 'ongoing';

          $data=mysqli_query($con,"UPDATE orders SET `paid`='$paid',`datestart`='$date',`dateend`='$enddate',`status`='$status' where `id`='$id'")or die( mysqli_error($con) );


          $data=mysqli_query($con,"UPDATE car SET `book`='1'where `id`='$carid'")or die( mysqli_error($con) );



    header("location:porders.php"); // Redirecting To Other Page
}
  ...
  <div class="content-wrapper">
    ...
    <tr>
     ...
     <div class="card">
        ...
        $rows =mysqli_query($con,"SELECT * FROM orders where status='pending' LIMIT 1000" ) or die(mysqli_error($con));
        $n=0;
        $stotal=0;
